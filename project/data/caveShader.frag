...
void main()
{
	...
	lowp vec3 Ia = vec3(1.0);
	lowp vec3 Id = vec3(1.0);
	lowp vec3 Is = vec3(1.0);

	lowp vec4 ambientVar = vec4(Ka * Ia, 1.0);
    
    lowp vec4 diffuseVar = vec4(0.0);
    mediump vec3 n = normalVarying;
    mediump vec3 l = normalize(vec3(lightPositionViewSpace_0 - posVarying));
    
    lowp float intensity = dot(l, n);
    lowp vec3 diffuse = Kd * clamp(intensity, 0.0, 1.0) * Id;
    diffuseVar = vec4(clamp(diffuse, 0.0, 1.0), 1.0);
    /*
    lowp vec4 specularVar = vec4(0.0);
    if (intensity > 0.0)
    {	
        mediump vec3 v = normalize(vec3(EyePos - posVarying)) ;
        mediump vec3 h = normalize(l + v);
        
        mediump vec3 specular = Ks * clamp(pow(dot(n, h),Ns), 0.0, 1.0) * Is;
        specularVar = vec4(clamp(specular, 0.0, 1.0), 1.0);
    }*/

	lowp vec4 color = texture( DiffuseMap, vec2(texCoordVarying));

    gl_FragColor = (ambientVar + diffuseVar) * color; //+ specularVar;
}
